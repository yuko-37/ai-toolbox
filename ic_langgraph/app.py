...
with gr.Blocks() as ui:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, min_width=80):
            coach_model = gr.Dropdown(choices=state.COACH_MODELS,
                                      show_label=False, container=False,
                                      interactive=True)
            eval_model = gr.Dropdown(choices=state.EVAL_MODELS,
                                     show_label=False, container=False,
                                     interactive=True)
    ...
    ev_sysprompt_inp.submit(fn=partial(sysprompt_update, key='ENGLISH_PROMPT'), inputs=[ev_sysprompt_inp],
                            outputs=ev_sysprompt_inp)
    ev_sysprompt_update_btn.click(fn=partial(sysprompt_update, key='ENGLISH_PROMPT'), inputs=[ev_sysprompt_inp],
                                  outputs=ev_sysprompt_inp)
...
if __name__ == '__main__':
    ui.launch(inbrowser=False, css=load_css())